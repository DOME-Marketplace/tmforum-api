...
class ExtendedCancelProductOrderApiControllerTest {
    ...
    private ExtendedCancelProductOrderApiController controller;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.initMocks(this);
        controller = new ExtendedCancelProductOrderApiController(queryParser, validationService, repository, eventHandler, tmForumMapper);
    }
    ...
    @Test
    void deleteCancelProductOrder_whenDeleteDisabled_returns405() throws Exception {
        setDeleteEnabled(false);

        HttpResponse<Object> response = controller.deleteCancelProductOrder(VALID_ID).block();

        assertEquals(HttpStatus.METHOD_NOT_ALLOWED, response.getStatus());
    }

    @Test
    void deleteCancelProductOrder_whenDeleteEnabled_returns204() throws Exception {
